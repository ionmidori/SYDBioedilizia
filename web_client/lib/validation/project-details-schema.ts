...
export const projectDetailsSchema = z.object({
    ...
    footage_sqm: z.number({ invalid_type_error: "Inserisci la metratura" }).positive("La metratura deve essere maggiore di zero").max(100000),
    property_type: z.enum(["apartment", "villa", "commercial"]),
    address: addressSchema,
    budget_cap: z.number({ invalid_type_error: "Inserisci il budget" }).positive("Il budget deve essere maggiore di zero"),
