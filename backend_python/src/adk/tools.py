...
@instrumented_tool("suggest_multi_room_quote")
async def suggest_multi_room_quote(
    session_id: str,
    project_id: str,
    user_id: str,
    rooms_json: str,
) -> str:
    """Generates an aggregated multi-room renovation quote with cross-room optimizations.

    Analyzes each room independently (measurements, structural delta, InsightEngine),
    then merges into a unified quote applying: SKU deduplication, volume discounts,
    shared overhead, and smaltimento consolidation.

    Use this when the user mentions multiple rooms (e.g., "bagno e cucina",
    "tutta la casa", "3 stanze") and wants a single unified quote.

    Args:
        session_id: The current project/session identifier (required).
        project_id: Project ID for the quote.
        user_id: UID of the authenticated user.
        rooms_json: JSON array of room specs. Each room: {"room_id": "uuid", "room_label": "Bagno Principale", "room_type": "bagno", "media_urls": ["https://..."]}.
    """
    import json
    import logging as _logging
    import uuid
    ...
    try:
        ...
        if not isinstance(rooms, list) or not rooms:
            return "Errore: rooms_json deve essere un array JSON non vuoto di stanze."
    except (json.JSONDecodeError, TypeError) as exc:
        _logger.error("[MultiRoomTool] Invalid rooms_json: %s", exc)
        return "Errore nel formato JSON delle stanze. Formato atteso: [{\"room_id\": \"...\", \"room_label\": \"...\", \"room_type\": \"...\", \"media_urls\": [...]}]"

    # Ensure each room has a room_id
    for room in rooms:
    ...
    chat_history = await repo.get_context(session_id, limit=40)
    ...
    try:
        ...
        _logger.error("[MultiRoomTool] Analysis failed: %s", exc, exc_info=True)
        return (
            "Si è verificato un errore durante l'analisi multi-stanza. "
            "Riprova tra qualche minuto o descrivi le lavorazioni per un preventivo manuale."
        )

    if not quote:
        return (
            "Non è stato possibile analizzare nessuna delle stanze indicate. "
            "Verifica che le foto siano state caricate correttamente."
        )
    ...
    try:
        ...
        quote_ref = (
            ...
            .document(target_project_id)
            .collection("private_data")
            .document("quote")
        )
        await quote_ref.set(quote.model_dump(exclude_none=True))
    except Exception as exc:
        _logger.error("[MultiRoomTool] Firestore save failed: %s", exc)
        # Non-fatal — continue to return the quote summary

    # 5. Build Italian summary with per-room breakdown
    response = f"**Preventivo Multi-Stanza ({len(quote.rooms)} ambienti)**\n\n"

    for room in quote.rooms:
        response += f"### {room.room_label} ({room.room_type})\n"
        if room.floor_mq:
            response += f"  Pavimento: ~{room.floor_mq:.1f} mq"
        if room.walls_mq:
            response += f" | Pareti: ~{room.walls_mq:.1f} mq"
        if room.floor_mq or room.walls_mq:
            response += "\n"
        response += f"  Voci: {len(room.items)} | Subtotale stanza: €{room.room_subtotal:.2f}\n"
        if room.missing_info:
            response += f"  ⚠️ Info mancanti: {', '.join(room.missing_info[:2])}\n"
        response += "\n"

    # Aggregation adjustments summary
    if quote.aggregation_adjustments:
        total_savings = sum(a.savings for a in quote.aggregation_adjustments)
        response += "### Ottimizzazioni Aggregazione\n"
        for adj in quote.aggregation_adjustments:
            if adj.savings > 0:
                response += f"  • {adj.description} → risparmi €{adj.savings:.2f}\n"
        response += f"\n  **Risparmio totale aggregazione: €{total_savings:.2f}**\n\n"
    ...
    response += "### Dettaglio Voci Unificate\n"
...
suggest_multi_room_quote_adk = FunctionTool(suggest_multi_room_quote)
