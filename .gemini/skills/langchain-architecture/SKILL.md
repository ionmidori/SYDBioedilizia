...
# Note: Use generic 'task' instead of hardcoded strings to let LLM decide
...
### Isolation Testing for Nodes
Test individual nodes by mocking the state, ensuring logic correctness without running the full graph.
...
async def test_retrieve_node():
    # Mock retrieval logic or dependencies
...
Use `interrupt_before` or `interrupt_after` to pause execution for user input.
...
# Interrupt before execution to allow review
...
# 2. Update state (if user changes plan)
await graph.aupdate_state(config, {"plan": "New Plan"})
...
- [ ] **Unit Test Nodes**: Test complex nodes in isolation
- [ ] **Human-in-the-Loop**: Use interrupts for sensitive actions
